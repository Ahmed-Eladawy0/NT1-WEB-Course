...
function levelDotsHtml(level) {
  ...
  if (l.includes("inter")) {
    filled = 2;
  } else if (l.includes("expert") || l.includes("adv")) {
    filled = 3;
  } else {
    filled = 1;
  }
  ...
  let dots = "";
  for (let i = 0; i < 3; i++) {
    const activeClass = i < filled ? "on" : "";
    dots += `<span class="${activeClass}"></span>`;
  }
  
  return `<span class="level-dots">${dots}</span><span class="level-label">${escapeHtml(level || "Beginner")}</span>`;
}
